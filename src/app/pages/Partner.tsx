...
export function Partner() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setStatus("sending");
    try {
      await sendViaEmailJS({ to_email: "afronated@gmail.com", from_name: formData.name, from_email: formData.email, organization: formData.organization, type: formData.type, message: formData.message, subject: `Partnership inquiry from ${formData.name}${formData.organization ? ` (${formData.organization})` : ""}` });
